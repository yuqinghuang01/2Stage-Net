...
unet.load_state_dict(torch.load(
    os.path.join(root_dir, "usz_ica_unet_fold1.pth")))
gccm.load_state_dict(torch.load(
    os.path.join(root_dir, "usz_ica_gccm_fold1.pth")))
ours.load_state_dict(torch.load(
    os.path.join(root_dir, "usz_ica_ours_fold1.pth")))
...
for (model, method_str) in zip(models, methods):
    ...
    with torch.no_grad():
        for i, data in enumerate(test_dataloader):
            ...
            output = np.pad(gt, (1,1), 'constant', constant_values=0)
            ...
            print(output.shape)
            np_to_stl(output, f'./test_stl/{name}_seg_{method_str}.stl')
            print(gt.shape)
            np_to_stl(gt, f'./test_stl/{name}_gt.stl')
